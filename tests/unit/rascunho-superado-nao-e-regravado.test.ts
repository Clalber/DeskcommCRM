...
async function salvar(versoes: VersaoRow[], publishedVersionId: string | null) {
  ...
  // ⚠️ O TERCEIRO ARGUMENTO É NOSSO, e não existe no upstream de onde este
  // arquivo veio. `saveAgentDraftAction` recebe o cadastro do agente (nome,
  // descrição, prioridade) separado do payload da VERSÃO, porque essas três
  // colunas moram em `ai_agents` e não em `ai_agent_versions` — ver o comentário
  // longo em `_actions.ts`, sobre os campos que sumiam sem erro nenhum.
  //
  // `{}` e não `undefined`: a action RECUSA `undefined` de propósito (chamador
  // quebrado) e aceita `{}` como "nada a mudar no cadastro", que é exatamente o
  // que este arquivo quer dizer — ele mede EM QUAL VERSÃO a escrita cai, e o
  // cadastro não entra nessa pergunta. Sem isto as cinco medições respondiam
  // `invalid_request` antes de chegar à escolha do rascunho, e o arquivo ficava
  // vermelho por um motivo que não é o que ele existe para vigiar.

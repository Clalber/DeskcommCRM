...
- **Espanhol é coberto, nos dois níveis** (PR #416, de @JowaniOrantes). Esta linha dizia *"lacuna conhecida: espanhol não é coberto — medido 0 de 9"*, e ficou falsa no merge daquele PR: o #275 tinha coberto só o vocabulário inequívoco, e o #416 acrescentou a camada ambígua e as construções com pronome preso (`escribirme`, `mandarme`). O número sai daqui de propósito — número envelhece, comando não: `grep -cE 'deja de|dejen de|no quiero|dame de baja' tests/unit/opt-out-deteccao.test.ts` conta as frases em espanhol sob teste.

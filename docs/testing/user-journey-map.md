...
> **Dívida conhecida (J1.21, ABERTO):** o agente do onboarding continua nascendo `kind='rag_bot'`, então abre no editor legado (Temperature, Top K, Similarity threshold) e as capacidades acima ficam **invisíveis** para o dono — funcionam no runtime, mas não têm superfície de configuração (invariante 6 do Sistema Vivo). Virar para `mcp_agent` hoje é pior: o editor novo exige `credential_id` para o formulário ficar válido (`AgentForm.tsx`) e `versionShapeSchema` o tipa como UUID obrigatório, enquanto instalação pelo kit funciona com a chave de plataforma do `.env` e não tem nenhuma linha em `ai_provider_credentials`. O dono cairia numa tela onde não consegue salvar nada. Depende de o editor aceitar a chave da instalação.

...
- **Os cards do fluxo passam a contar o que fazem** No desenho de um follow-up, o primeiro card dizia **«Início do fluxo»** — o mesmo texto para todos os tipos de gatilho. Era a caixa mais importante do fluxo e a única que não dizia nada: o que dispara, de quanto em quanto tempo, e por causa do quê ficava num botão no topo da tela, longe do desenho.

  Agora cada card responde as perguntas que ele mesmo levanta:

  **O card do gatilho** diz o que olha e quando dispara — «Quando faltar 1 hora para um compromisso», «olha a Agenda, a cada minuto» — e avisa das regras que agem sozinhas, como o compromisso entrar uma vez só e voltar a valer se for remarcado. Vale para os seis gatilhos, cada um com a sua frase.

  **O card de aviso** diz **para quem** vai a mensagem (e isso vem do gatilho, não do card) e **por onde** ela sai. O texto aparece com um exemplo das variáveis já resolvidas: em vez de `{{agendamento.hora}}`, você lê «às 14:00».

  **O card de fim** diz o que fica registrado, em vez do tipo do desfecho.

  **As setas de caminho único** deixam de dizer «Sempre» e passam a dizer o que acontece ali: «assim que disparar», «depois de enviar», «passado o tempo». Onde o nó ramifica de verdade — o classificar, a condição — o nome do ramo continua como estava, porque ali ele já dizia algo.

  E quando o fluxo é de lembrete de compromisso, um aviso no rodapé do desenho lembra que ele só alcança os tipos de atendimento com o lembrete ligado — com link para a tela onde se liga.

  Nada mudou no comportamento: é o que a tela conta sobre o que já acontecia.
...
- **O follow-up avisa o cliente antes de um compromisso marcado** O construtor de fluxo ganhou um gatilho novo: **Antes de um compromisso marcado**. Escolha com quanta antecedência o fluxo começa — uma hora, um dia — e ele inscreve sozinho quem tem hora marcada na agenda, a tempo de a mensagem chegar antes.
...
- **Criar uma automação deixa de gerar duas iguais** Criar uma automação podia produzir duas idênticas. Não era clique repetido: quando a rede engasga no meio do envio, o navegador reenvia o mesmo pedido sozinho — e a tela de automações não reconhecia que era o mesmo pedido, criando outra regra.
...
- **O atendimento automático para de mandar duas mensagens seguidas sem ninguém ter escrito** Quem escrevia duas mensagens em sequência às vezes recebia **duas respostas** —
...
- **O atendimento automático para de negar uma reunião que ele mesmo marcou** O agente de IA marcava a reunião, confirmava ao cliente — e minutos depois, se a
...
- **Responder pelo Instagram volta a funcionar** Toda resposta enviada numa conversa do Instagram falhava dizendo que o
...
- **Cadastrar o webhook do Instagram na Meta volta a funcionar** Ao cadastrar a URL de webhook no painel da Meta, ela recusava dizendo que
...
- **Atender o Instagram Direct no mesmo Inbox do WhatsApp** As mensagens diretas do Instagram passam a chegar no mesmo Inbox, com as mesmas
  conversas, o mesmo agente de IA e o mesmo funil. Em **Conexões → Instagram** você
  guarda as credenciais do aplicativo, copia os dois endereços para cadastrar no
  painel da Meta e autoriza a conta.

  **O que preparar antes.** Cada empresa cria o próprio aplicativo na Meta — não
  existe um aplicativo único para todo mundo, porque a conta que responde é a sua.
  É preciso uma conta profissional do Instagram, um portfólio de negócios
  verificado, e a aprovação da Meta, **que leva de semanas a meses**. Vale começar
  por ela.

  **Três diferenças em relação ao WhatsApp**, e é melhor saber antes de vender
  atendimento por aqui:

  - A conversa fica aberta por **24 horas** contadas da última mensagem do cliente.
    Passado esse prazo não há modelo aprovado para reabrir, como existe no
    WhatsApp: quem responde tem que ser uma pessoa, e o agente de IA encerra o
    turno e escala.
  - **Não há grupos**, e a mensagem não é cobrada por unidade.
  - **A credencial vence a cada 60 dias.** Esta é a única do produto que vence. A
...
[Não lançado]: https://github.com/melgarafael/DeskcommCRM/compare/v2.11.1...HEAD
[2.11.1]: https://github.com/melgarafael/DeskcommCRM/compare/v2.11.0...v2.11.1

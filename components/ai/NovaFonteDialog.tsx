...
export function NovaFonteDialog({ agentId, tipo, rotulo, aberto, onFechar, onCriada }: Props) {
  const t = useT();
  const [nome, setNome] = useState(`${rotulo} ${t("da loja")}`);
  ...
  return (
    <Dialog open={aberto} onOpenChange={(v) => !v && onFechar()}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>{t("Cadastrar")} {rotulo.toLowerCase()}</DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {/* O placeholder e os marcadores `## Pergunta:` / `## Resposta:` ficam
                em português de propósito: `lib/ai/rag/ingest/faq.ts` reconhece só
                esse texto literal (regex fixa) — traduzir aqui mostraria um
                exemplo que a API não consegue interpretar. */}
            <Textarea
              id="fonte-conteudo"
              rows={12}
              placeholder={EXEMPLO}

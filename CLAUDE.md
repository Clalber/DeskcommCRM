...
9. **Função nova em `public` nasce EXPOSTA — revogue as TRÊS origens.** Toda `create function` no schema `public` termina com:

   ```sql
   revoke execute on function public.fn_x(...) from public, anon, authenticated;
   grant  execute on function public.fn_x(...) to <só quem precisa>;
   ```

   São três origens distintas de `EXECUTE`, e tratar só algumas deixa a função exposta com o gate verde: **(A)** o grant direto a `anon` do `ALTER DEFAULT PRIVILEGES ... GRANT ALL ON FUNCTIONS TO anon` do baseline, que vale para toda função criada depois dele — isto é, para todo apêndice novo — e que `revoke from public` **não** remove; **(B)** o grant a `PUBLIC` que o Postgres dá a qualquer função ao criá-la, que `revoke from anon` **não** remove; **(C)** o mesmo `ALTER DEFAULT PRIVILEGES` para **`authenticated`**, que nenhum dos dois anteriores remove.

   **Esta linha dizia "as DUAS origens" e omitia (C) até 2026-08-30.** O custo foi medido no mesmo dia: `fn_upsert_conversation_do_canal` (migration 0207) seguiu a instrução ao pé da letra, revogou de `public, anon`, e nasceu executável por **qualquer usuário logado de qualquer organização** — numa função que recebe a organização como PARÂMETRO, ou seja, escrita cross-tenant com a sessão do próprio atacante. Quem pegou foi `tests/invariants/hardening-definer-varredura.test.ts`, no CI, porque a varredura mede o banco e não a prosa. A função irmã (`fn_upsert_wa_conversation`) já revogava das três — mas em **três blocos distantes** do baseline, e escrita assim a terceira não se parece com regra. Para ver o idioma em vigor sem confiar nesta linha: `grep -c "from public, anon, authenticated" supabase/baseline.sql`.

   Sem os três, o PostgREST expõe a função como RPC alcançável pela anon key (que vai para o browser) ou pelo JWT de qualquer usuário logado. Vigiado por `tests/invariants/hardening-definer-varredura.test.ts`, que varre todas as `security definer` de `public` e tem uma lista `AUTHENTICATED_PERMITIDO` **com o call site nomeado** para as exceções legítimas (issue #128 — a versão anterior checava uma lista fixa de 6, e 8 de 25 estavam expostas).

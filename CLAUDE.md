...
- **`e2e`** (`e2e.yml`) — sobe Supabase local, aplica o `baseline.sql` e roda **44 das 45 specs** Playwright (medido em 2026-08-12; **reconte antes de citar**, com `ls tests/e2e/*.spec.ts | wc -l` e parseando as três listas do `e2e.yml` — este número já apodreceu **três** vezes: uma delas fui eu copiando o `echo` do próprio workflow em vez de contar os arquivos, e na terceira o texto ainda listava `agente-papeis-operador` como fora quando ela já estava em `SPECS_PARTE_1`). A **única** de fora é `vps-fresh-onboarding` (precisa de WAHA + Redis + Resend + Nuvemshop), e ela é a **P0** da doutrina de QA Visual — ou seja, `e2e` verde **não** prova a jornada de instalação fresca, que é o produto que se vende.

  Se este número apodrecer uma quarta vez, o conserto não é recontar: é `tests/unit/e2e-cobertura-completa.test.ts` passar a cobrar também o texto daqui, como já cobra as três listas do workflow. Prosa que nenhum gate lê é prosa que diverge — e uma triagem que a use como régua mede contra o número errado, que é o modo de falha nº 1 do procedimento.
